...
router.post("/", requireRole("INSTRUCTOR"), async (req, res) => {
  ...
  if (!lesson || lesson.course.instructorId !== req.auth!.userId) return res.status(404).json({ message: "Lesson not found" });
  if (lesson.quiz) return res.status(409).json({ message: "Quiz already exists for this lesson" });
